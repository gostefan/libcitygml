...
char const* fillFromString(std::string_view view, std::array<T*, N> const& targets) {
...
template <typename T, size_t N>
void readValues(std::istream& is, std::array<T*, N> const& targets) {
    std::string input;
    char const* end;
    for ( T* target : targets) {
        if (is >> input) {
...
template<class T> char const* TVec2<T>::fromString(std::string_view view) {
    return fillFromString(view, std::array<T*, 2>{ &x, &y });
}

template<class T> inline std::istream& operator>>(std::istream & is, TVec2<T> & v)
{
    readValues(is, std::array<T*, 2>{ &v.x, &v.y });
    return is;
}
...
template<class T> char const* TVec3<T>::fromString(std::string_view view) {
    return fillFromString(view, std::array<T*, 3>{ &x, &y, &z });
}

template<class T> inline std::istream& operator>>(std::istream & is, TVec3<T> & v) {
    readValues(is, std::array<T*, 3>{ &v.x, &v.y, &v.z });
    return is;
}
...
template<class T> char const* TVec4<T>::fromString(std::string_view view) {
    return fillFromString(view, std::array<T*, 4>{ &x, &y, &z, &w });
}

template<class T> inline std::istream& operator>>( std::istream & is, TVec4<T> & v )
{
    readValues(is, std::array<T*, 4>{ &v.x, &v.y, &v.z, &v.w });
    return is;
}
